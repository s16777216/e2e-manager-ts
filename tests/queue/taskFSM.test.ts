...
describe("任務狀態有限狀態機單元測試 (taskFSM.ts)", () => {
  ...
  it("2. TaskFSM.complete (PASS): 應回傳 passed 狀態、PASS 結果與結束時間", () => {
    const now = new Date();
    const payload = TaskFSM.complete("PASS", "測試成功通過", now);

    expect(payload.status).toBe("passed");
    expect(payload.finalResult).toBe("PASS");
    expect(payload.finalReason).toBe("測試成功通過");
    expect(payload.finishedAt).toBe(now);
  });

  it("3. TaskFSM.complete (FAIL): 應回傳 failed 狀態、FAIL 結果與結束時間", () => {
    const now = new Date();
    const payload = TaskFSM.complete("FAIL", "頁面未呈現預期元素", now);

    expect(payload.status).toBe("failed");
    expect(payload.finalResult).toBe("FAIL");
    expect(payload.finalReason).toBe("頁面未呈現預期元素");
    expect(payload.finishedAt).toBe(now);
  });

  it("4. TaskFSM.crash: 應回傳 error 狀態、ERROR 結果、錯誤說明與結束時間", () => {
    const now = new Date();
  ...
  it("5. TaskFSM.timeout: 應回傳 failed 狀態、FAIL 結果、超時幾分鐘理由與結束時間", () => {
    const now = new Date();
