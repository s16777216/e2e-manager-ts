...
背景任務執行器 SHALL 實作非同步先進先出 (FIFO) 佇列。佇列狀態 MUST 儲存於資料庫的 `test_runs` 表中，且執行器 MUST 使用資料庫交易行級鎖定（如 `FOR UPDATE SKIP LOCKED`）安全地領取任務，確保多個 Worker 平行運作時不會發生重複領取任務的 race condition，且併發執行數限制為 1。
