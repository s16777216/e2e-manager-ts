...
- **THEN** 前端即時將接收到的日誌更新至對應的步驟區塊中，以折疊時間軸展示動作細節，且在收到包含步驟截圖時更新並常駐顯示於該步驟區塊下方，點擊返回按鈕時正確回到該測試案例的詳情頁
