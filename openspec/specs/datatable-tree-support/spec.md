## Capability: datatable-tree-support

通用 `DataTable` 元件支援選配的樹狀階層分層（Tree Grid）展開/收合能力，與現有一維平鋪表格完全向下相容。
...
#### Scenario: Pagination with expanded rows
- **WHEN** 樹狀表格啟用分頁且有展開的子列時
- **THEN** 展開的子列 SHALL 固定顯示在父列正下方，不因分頁邊界而與父列分離（`paginateExpandedRows: false`）。
