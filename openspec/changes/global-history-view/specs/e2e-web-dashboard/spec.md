...
系統 MUST 在側邊欄提供首頁、專案列表、執行紀錄（History）三個主要入口導航。專案詳細頁面中 MUST 提供群組與測試案例樹狀結構表格（Tree Table）導航，以列與欄的形式展示資訊。點選特定群組時，系統 SHALL 僅展開或收合該節點。專案詳細頁面 MUST 以單欄滿版表格（Tree Table）的形式填滿主要工作區。專案詳細頁面中 SHALL 移除原本位於底部的歷史批次執行表格，將執行紀錄的功能統一收歸於全域執行紀錄頁面中。
...
- **WHEN** 載入應用程式時
- **THEN** 側邊欄 MUST 顯示首頁、專案列表及執行紀錄入口，且點擊執行紀錄入口時能正確 navigate 導航至 `/tasks`
